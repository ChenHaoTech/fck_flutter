...
class GetInstance {
  ...
  static Iterable<String> get singlKey => _singl.keys;
  ...
  S find<S>({String? tag}) {
    assert(_singl.length <= 500, "可能内存泄漏? ${_singl.keys.join(", ")}");
    final key = _getKey(S, tag);
  ...
  bool delete<S>({String? tag, String? key, bool force = false}) {
    ...
    if (!_singl.containsKey(newKey)) {
      Get.log('Instance(${S.runtimeType}) "$newKey" already removed.', isError: true);
      return false;
    }
