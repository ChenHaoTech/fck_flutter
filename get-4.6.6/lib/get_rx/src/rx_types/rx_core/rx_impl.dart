...
mixin RxObjectMixin<T> on NotifyManager<T> {
  ...
  T get justValue => _value;

  set justValue(T value) {
    _lastValue = _value;
    _value = value;
  }
  ...
  T get value {
    RxInterface.proxy?.addListener(subject, debug);
    return _value;
  }
  ...
  void bindRx<E>(RxObjectMixin<E> rx, void Function(E) callback) {
    final listSubscriptions = _subscriptions[subject] ??= <StreamSubscription>[];
    callback.call(rx.justValue);
    listSubscriptions.add(rx.listen((va) => callback.call(va)));
  }

  final List<RxObjectMixin> children = [];
  @override
  void close() {
    super.close();
    children.forEach((rx) => rx.close());
  }

  RxObjectMixin<T> bind(DisposableInterface disposableInterface) {
    disposableInterface.disposeFunc.add(() => this.close());
    return this;
  }
}
...
mixin NotifyManager<T> {
  ...
  void addListener(GetStream<T> rxGetx, String? debug) {
    if (!_subscriptions.containsKey(rxGetx)) {
      if (this.debug != null) {
        trace.add(debug ?? rxGetx.toString());
      }
...
abstract class _RxImpl<T> extends RxNotifier<T> with RxObjectMixin<T> {
  ...
  T get justValue => _value;
  set justValue(value) => _value = value;
